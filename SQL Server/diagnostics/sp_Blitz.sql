/*
 Быстрая проверка вашего SQL Server (скрипт)

Сегодня запущена новая рубрика «Скрипты». В данной рубрике будут публиковаться готовые скрипты, которые можно выполнять на любом сервере без дополнительной обработки.

И первый скрипт будет от команды Brent Ozar Unlimited под названием sp_Blitz

Вам достался Microsoft SQL Server от другого администратора или же вы не глубоко знаете данную технологию. Вы хотите быстро и просто получить максимальную информацию о состоянии SQL Server. Тогда вам поможет скрипт sp_Blitz
...
Обычно процедура выполняется от 5 до 15 секунд. Скрипт максимально проработан чтобы не препятствовать работе других пользователей, но вам всё равно лучше проверить его работу на тестовом сервере. Для первого запуска достаточно вызвать процедуру без параметров, но вам наверняка захочется получить более детальную информацию. Для этого необходимо прочитать комментарии в самом скрипте.
*/
USE [master];
GO
