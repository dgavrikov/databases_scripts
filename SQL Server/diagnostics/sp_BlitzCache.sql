/*
Анализ медленных запросов в кэше (скрипт)

” вас есть SQL Server и вы не знаете какие запросы вызывают максимальные проблемы с производительностью? ¬ы не знаете с чего начать или у вас есть скрытые запросы?

вы хотите быстрое и качественное решение которое сможет обнаружить медленные запросы в пам¤ти и сообщит вам почему именно указанные запросы плохи?
*/
USE master;
GO
