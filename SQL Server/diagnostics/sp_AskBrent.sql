/*
Что мешает работе вашего SQL Server в данный момент.
Часто ли вам задают вопрос: «SQL Server работает медленно, что вызывает торможение системы?»

Существует множетсво пунктов, которые необходимо проверить:

1.  Посмотреть sp_who, sp_who2, найти блокирующие и ожидающие запросы

2. Проверить нет ли активности от backup, планов обслуживания или проверок целостности БД

3. Найти кто именно вызывает текущие блокировки

4. Посмотреть на счётчики производительности чтобы понять ситуацию более детально

Каждый раз нам приходится выполнять огромную работу чтобы ответить на данный вопрос, но теперь вы можете пользоваться скриптом sp_AskBrent, который поможет вам быстроо понять всю ситуацию быстро и решить проблему.
