/*
Анализ полезности индексов (скрипт)

Вами индексы могут быть не такими хорошими, как вы думаете. Если вы хотите узнать:

1. Есть ли у вас дубликаты индексов, которые тратят ваше дисковое пространство и память?

2. Присутствуют ли неиспользуемые индксы?

3. Где используются большие кластерные индексы, которые включаются во множество некластерныъ индексов?

4. Наличие активных куч, которые вызывают «странную» фрагментацию?

То вам безусловно будет полезен скрипт sp_BlitzIndex, который позволит быстро и качественно проанализировать индексы. Каждый результат будет ссылаться на web, где будут даны пояснения (англ).

Если вы когда-то задумывались о тюнинге ваших индексов, то данный скрипт поможем вам.
*/
SET ANSI_NULLS ON;
SET ANSI_PADDING ON;
